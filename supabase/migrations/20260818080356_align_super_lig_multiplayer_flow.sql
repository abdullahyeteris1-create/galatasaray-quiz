...
-- Keep v3 available for existing clients and add capacity without changing the
-- identity of any deployed function.
create or replace function public.quiz_super_lig_create_room_v4(
  p_name text,
...
declare
  ...
  v_exclude bigint[];
begin
  if char_length(trim(p_name)) < 1 or char_length(trim(p_name)) > 24 then
    raise exception 'INVALID_NAME';
  end if;
  if p_max_players < 2 or p_max_players > 12 then
    raise exception 'INVALID_CAPACITY';
  end if;
  if p_era not in ('mixed', '2000s', '2010s', '2020s') then
    raise exception 'INVALID_ERA';
  end if;
  if p_question_count not in (10, 15, 20) then
    raise exception 'INVALID_QUESTION_COUNT';
  end if;

  select coalesce(array_agg(x order by n desc), '{}')
  into v_exclude
  from (
    select distinct x, n
    from unnest(coalesce(p_exclude_question_ids, '{}')) with ordinality as u(x, n)
    where n <= 50
  ) recent;
  ...
  loop
    ...
    begin
      insert into public.quiz_rooms (
        ...
        game_type,
        super_lig_era,
        super_lig_exclude_question_ids_v2
      )
      values (
        v_code,
        'waiting',
...
begin
  ...
  where code = upper(trim(p_code)) and game_type = 'super_lig'
  for update;
  ...
  if exists (
    ...
    where room_id = v_room.id and lower(display_name) = lower(trim(p_name))
  ) then
...
begin
  ...
  using public.quiz_rounds r
  where a.round_id = r.id and r.room_id = p_room;
  ...
  select p_room, row_number() over ()::smallint, id
  from (
    select q.id
    from public.quiz_questions q
    where q.active
  ...
  if v_pick_count < v_room.question_count then
    delete from public.quiz_room_questions where room_id = p_room;
    insert into public.quiz_room_questions (room_id, position, question_id)
    select p_room, row_number() over ()::smallint, id
    from (
  ...
  where q.id in (
    select question_id from public.quiz_room_questions where room_id = p_room
  );
  ...
  from public.quiz_room_questions
  where room_id = p_room and position = 1;
...
create or replace function public.quiz_super_lig_tick(
  p_room uuid,
  p_player uuid,
  p_token text
) returns jsonb
...
begin
  select exists (
    ...
    where p.id = p_player
      ...
      and r.game_type = 'super_lig'
      and p.join_token_hash = encode(extensions.digest(p_token, 'sha256'), 'hex')
  )
  into v_ok;
  ...
  if v_room.status <> 'playing' then
    return jsonb_build_object('status', v_room.status, 'current_round', v_room.current_round);
  end if;

  select *
  into v_round
  from public.quiz_rounds
  where room_id = p_room and round_number = v_room.current_round
  for update;
  ...
  if v_now >= v_round.ends_at and v_round.revealed_at is null then
    ...
    where a.round_id = v_round.id and a.is_correct
    ...
    set points_awarded = case when player_id = v_winner then 1 else 0 end
    where round_id = v_round.id;
    update public.quiz_players p
    set correct_count = p.correct_count + (
          select count(*)
          from public.quiz_answers a
          where a.round_id = v_round.id and a.player_id = p.id and a.is_correct
        ),
        score = p.score + case when p.id = v_winner then 1 else 0 end
    where p.room_id = p_room;
    v_round.revealed_at := v_now;
  end if;

  if v_round.revealed_at is not null
     and v_now >= v_round.revealed_at + interval '4 seconds' then
    ...
    select p_room, v_next, question_id, v_start, v_start + interval '15 seconds'
    from public.quiz_room_questions
    where room_id = p_room and position = v_next
...
create or replace function public.quiz_super_lig_get_state(
  p_room uuid,
  p_player uuid,
  p_token text
) returns jsonb
...
begin
  ...
  select * into v_room
  from public.quiz_rooms
  where id = p_room and game_type = 'super_lig';
  if not found then raise exception 'ROOM_NOT_FOUND'; end if;
  ...
  if v_room.status = 'playing' and v_room.current_round > 0 then
    ...
    if found then
      ...
      select exists (
        ...
        where round_id = v_round.id and player_id = p_player
      )
      into v_answered;
        ...
        v_payload := v_payload || jsonb_build_object(
          'reveal', jsonb_build_object(
            ...
            'winner_id', (
              select a.player_id
              from public.quiz_answers a
              where a.round_id = v_round.id and a.points_awarded = 1
              order by a.answered_at, a.id
              limit 1
            )
          )
...
revoke all on function public.quiz_super_lig_create_room_v4(
  text,
  smallint,
  text,
  smallint,
...
grant execute on function public.quiz_super_lig_create_room_v4(
  text,
  smallint,
  text,
  smallint,
